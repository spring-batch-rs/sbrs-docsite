...
fn build_tolerant_step() -> Step {
    StepBuilder::new("process-with-skip")
        .chunk(100)
        .reader(&reader)
        .processor(&processor)
        .writer(&writer)
        .skip_limit(3)  // Skip up to 3 failed items
        .build()
}
```
...
graph TB
    ...
    Transient -->|Yes| UseRetry[Reduce Chunk Size<br/>or Fix Transient Issue]
