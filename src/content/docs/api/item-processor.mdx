...
<Aside type="tip">
  The processor takes the input item **by value**, enabling zero-cost pass-through when `I == O` — no allocation required.
</Aside>
...
A zero-cost, no-allocation processor that passes items through without transformation.
