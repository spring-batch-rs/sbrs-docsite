...
|--------|------|-------------|
| `postgres(&Pool<Postgres>)` | `&Pool<Postgres>` | PostgreSQL connection pool |
| `mysql(&Pool<MySql>)` | `&Pool<MySql>` | MySQL connection pool |
| `sqlite(&Pool<Sqlite>)` | `&Pool<Sqlite>` | SQLite connection pool |
| `table(&str)` | `&str` | Target table name |
| `column(name, extractor)` | `&str`, `impl Fn(&O) -> ColumnValue` | Map a field to a column |
| `build_postgres()` | — | Build a `PostgresItemWriter` |
| `build_mysql()` | — | Build a `MySqlItemWriter` |
| `build_sqlite()` | — | Build a `SqliteItemWriter` |
...
<Aside type="note">
  The writer uses bulk INSERT for optimal performance. The `ColumnValue` enum handles type mapping: `i32`/`i64` → `Int`, `f32`/`f64` → `Float`, `bool` → `Bool`, `&str`/`String` → `Text`, `Vec<u8>` → `Bytes`, `Option<T>` → `Null` when `None`.
</Aside>
...
Same `RdbcItemWriterBuilder`, select MySQL with `.mysql(&pool)`:
...
Same `RdbcItemWriterBuilder`, select SQLite with `.sqlite(&pool)`:
