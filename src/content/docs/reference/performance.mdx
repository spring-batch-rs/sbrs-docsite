...
let writer = RdbcItemWriterBuilder::<Record>::new()
    .postgres(&pool)
    .table("target_table")
    .column("field1", |r: &Record| r.field1.as_str().into())
    .column("field2", |r: &Record| r.field2.as_str().into())
    .column("field3", |r: &Record| r.field3.into())
    .build_postgres();
...
```rust
// Inefficient: Cloning after taking by value
impl ItemProcessor<Record, Record> for MyProcessor {
    fn process(&self, item: Record) -> ItemProcessorResult<Record> {
        let mut owned = item;
        owned.field = transform(owned.field);
        Ok(Some(owned))
    }
}

// Efficient: Transform and move
impl ItemProcessor<Record, Record> for MyProcessor {
    fn process(&self, item: Record) -> ItemProcessorResult<Record> {
        Ok(Some(Record {
...
let writer = RdbcItemWriterBuilder::<Record>::new()
    .postgres(&small_pool)  // 2 connections only
    .table("target")
    .column("field1", |r: &Record| r.field1.as_str().into())
    .build_postgres();
...
let writer = RdbcItemWriterBuilder::<Record>::new()
    ...
    .table("target")
    .column("field1", |r: &Record| r.field1.as_str().into())
    .column("field2", |r: &Record| r.field2.into())
    .build_postgres();
