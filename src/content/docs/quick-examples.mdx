  ...
  <TabItem label="Complete Example">
    ...
    async fn migrate_users() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let writer = RdbcItemWriterBuilder::<ModernUser>::new()
            .mysql(&mysql_pool)
            .table("users")
            .column("id", |u: &ModernUser| u.id.into())
            .column("username", |u: &ModernUser| u.username.as_str().into())
            .column("email", |u: &ModernUser| u.email.as_str().into())
            .column("created_timestamp", |u: &ModernUser| u.created_timestamp.into())
            .column("migrated_at", |u: &ModernUser| u.migrated_at.into())
            .build_mysql();

        // Build and execute step
        let step = StepBuilder::new("migrate-users")
            .chunk::<LegacyUser, ModernUser>(500)
            .reader(&reader)
            .processor(&processor)
            .writer(&writer)
            .build();
