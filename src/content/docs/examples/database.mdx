...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Write to PostgreSQL
    let writer = RdbcItemWriterBuilder::<Person>::new()
        .postgres(&pool)
        .table("persons")
        .column("first_name", |p: &Person| p.first_name.as_str().into())
        .column("last_name", |p: &Person| p.last_name.as_str().into())
        .column("email", |p: &Person| p.email.as_str().into())
        .column("birth_date", |p: &Person| p.birth_date.as_str().into())
        .build_postgres();
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let writer = RdbcItemWriterBuilder::<ProcessedPerson>::new()
        .postgres(&pool)
        .table("processed_persons")
        .column("id", |p: &ProcessedPerson| p.id.into())
        .column("first_name", |p: &ProcessedPerson| p.first_name.as_str().into())
        .column("last_name", |p: &ProcessedPerson| p.last_name.as_str().into())
        .column("email", |p: &ProcessedPerson| p.email.as_str().into())
        .column("email_domain", |p: &ProcessedPerson| p.email_domain.as_str().into())
        .build_postgres();
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let writer = RdbcItemWriterBuilder::<TransactionFact>::new()
        .postgres(&dw_pool)
        .table("fact_transactions")
        .column("transaction_key", |f: &TransactionFact| f.transaction_key.into())
        .column("customer_key", |f: &TransactionFact| f.customer_key.into())
        .column("product_key", |f: &TransactionFact| f.product_key.into())
        .column("date_key", |f: &TransactionFact| f.date_key.into())
        .column("quantity", |f: &TransactionFact| f.quantity.into())
        .column("unit_price", |f: &TransactionFact| f.unit_price.into())
        .column("total_amount", |f: &TransactionFact| f.total_amount.into())
        .column("gross_profit", |f: &TransactionFact| f.gross_profit.into())
        .build_postgres();
